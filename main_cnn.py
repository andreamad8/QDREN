"""
Usage:
    main.py [--batch_size=<kN>] [--embedding_size=<kN>] [--num_blocks=<kN>] [--epoch=<kN>] [--task_num=<kN>] [--learning_rate=<kN>]
    main.py -h | --help
...
import os
import time
from src.utily_cnn import Dataset
from mem_cell import DynamicMemoryCell
from functools import partial
from simpleEntNet import entNet
import numpy as np
...
def main(arguments):
    ...
    summary_writer_train = tf.train.SummaryWriter('logs/train/task', sess.graph)
    summary_writer_val = tf.train.SummaryWriter('logs/val/task')
