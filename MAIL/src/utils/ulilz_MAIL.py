...
def get_train_test(train_size,dev_size,test_size,sent_len,sent_numb,embedding_size,max_windows, win):
    ...
    train_examples = load_data('data/mail/train.txt', train_size, relabeling=True)

    logging.info('*' * 10 + ' Dev')
    dev_examples = load_data('data/mail/dev.txt', dev_size, relabeling=True)

    logging.info('*' * 10 + ' Test')
    test_examples = load_data('data/mail/test.txt', test_size, relabeling=True)
