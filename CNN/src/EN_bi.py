...
class EntityNetwork():
    ...
    def instantiate_weights(self):
        ...
        self.H = tf.get_variable("H", [2 * self.embedding_size, self.embedding_size], initializer=self.init)
    ...
    def inference(self):
        ...
        # Create Memory Cell
        self.cell_FW = DynamicMemoryCell(self.num_blocks, self.embedding_size,
                                      self.keys, query_embedding)
        self.cell_BW = DynamicMemoryCell(self.num_blocks, self.embedding_size,
                                      self.keys, query_embedding)
        # self.cell =tf.contrib.rnn.DropoutWrapper(self.cell, output_keep_prob=self.keep_prob)

        # Send Story through Memory Cell
        initial_state_FW = self.cell_FW.zero_state(self.batch_size, dtype=tf.float32)
        initial_state_BW = self.cell_BW.zero_state(self.batch_size, dtype=tf.float32)

        self.out, memories = tf.nn.bidirectional_dynamic_rnn(self.cell_FW,self.cell_BW,
                                        story_embeddings,
