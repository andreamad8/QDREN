...
for step in range(100):
    for i,j in data.get_batch_train(900):
        dic = data.get_dic_train(S_input,Q_input,A_input,i,j)
        _, loss_train, acc_train = sess.run([train_op, loss, accuracy], feed_dict=dic)
    if step % 10 == 0:
        dic = data.get_dic_val(S_input,Q_input,A_input)
        loss_val, acc_val = sess.run([loss, accuracy], feed_dict=dic)
